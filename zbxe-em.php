...
/* Used for inicial development: "events.php" from Zabbix INC
 * * Objective: Interrelationships between events (temporal analysis of possible causes and effects)
...
$source = getRequest('triggerid') > 0 ? EVENT_SOURCE_TRIGGERS : getRequest('source', CProfile::get('web.events.source', EVENT_SOURCE_TRIGGERS));
...
if ($mode !== "report") {
    $events_wdgt->addPageHeader(
            _zeT('Report generated on') . SPACE . '[' . zbx_date2str(_('d M Y H:i:s')) . ']', array(
        $frmForm,
        SPACE,
...
if (EVENT_SOURCE_TRIGGERS == $source) {
    ...
    $unkcbx = new CCheckBox('hide_unk', $_REQUEST['showUnknown'], 'javascript: create_var("' . $filterForm->GetName() . '", "showUnknown", (this.checked?1:0), 0); ', '1');
...
//to_2.4.7                    $paging = getPagingLine($events);
                ...
                foreach ($events as $enum => $event) {
                    ...
                    $actions = new CButton("btnEM", _zeT('Correlate'), "return PopUp('zbxe-em.php?p_mode=report&"
                    ...
                    $table->addRow(array(
                        new CLink(zbx_date2str($formato_data, $event['clock']), 'tr_events.php?triggerid=' . $event['objectid'] . '&eventid=' . $event['eventid'], 'action'
                        ),
                        (versaoZabbix() < 240 ? (is_show_all_nodes() ? get_node_name_by_elid($event['objectid']) : null) : null),
                        $hostSpan,
