...
//	var_dump($date);
	...
	$calendar_html .= "\n<tr><th class='tSemana'>Domingo</th><th  class='tSemana'>Segunda</th><th  class='tSemana'>Ter�a</th><th  class='tSemana'>Quarta</th><th  class='tSemana'>Quinta</th><th  class='tSemana'>Sexta</th><th  class='tSemana'>S�bado</th><tr>";
	...
	for ($day_counter = $date['mday']/*1*/; $day_counter <= $days_in_this_month; $day_counter++) {
	  $week_day %= 7;
	...
	//$row['id_plantonista'],$row['nm_plantonista'],$row['st_dias'],0)
	  ...
	  // Pontua��o ganha pelo dia de plantao ------------------------
          if ($day <= $day_counter) {
            $p_plan[$indice][3] += ($week_day == 6 /*|| */ ? 5 : ($week_day == 0 ? 0 : 1));//$pontoDia[$week_day];//
            if ("NNNNNNS" == $p_plan[$indice][2]) {
                    $p_plan[$indice][3] += 5;
            }
          }
	  $descricao = $day_counter."<br>".$p_plan[$indice][1].
	  ($vDebug ? "<br>Pontos=".$p_plan[$indice][3]."<br>[{$indice}]".$tmp11." - ds - ".$week_day 
	  : "");
	  if ($p_plan[$indice][0] >= 0) {
	  	$sql .= "INSERT INTO `".$DB_EXTRAS."`.`plantoes` (`dt_plantao` ,`userid` ,`userid_substituto` ,`st_status` ,`tx_motivo`) VALUES ('"
...
$query = "Select * from (SELECT pri.userid, pri.nm_plantonista, res.st_dias FROM "
        .$DB_EXTRAS.".`plantonistas` pri, "
        .$DB_EXTRAS.".`restricoes` res
...
$ANO = $_REQUEST['p_ano'];
//echo calendar(getDate(mktime(0,0,0,5,14,$ANO)),$plan);
//echo calendar(getDate(mktime(0,0,0,5,16,$ANO)),$plan);
for ($i = $_REQUEST['p_inicio']; $i < $_REQUEST['p_fim']; $i++) {
  echo "gerando para o mês " . $i ;
  echo calendar(getDate(mktime(0,0,0,$i,1,$ANO)),$plan);
}
?>
