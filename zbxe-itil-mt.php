<?php
/* Calculo de tempos médios de recuperação e de repetição de incidentes --------
...
Query que localiza, um a um, todos os eventos (menos o ultimo conjunto)
SELECT ev1.eventid AS id_inicio, ev1.clock AS clock_inicio, MIN( ev2.eventid ) id_final, ev2.clock AS clock_final, ev2.clock - ev1.clock AS tempo_ok, ev3.clock - ev2.clock AS tempo_problem
FROM  `events` ev1
INNER JOIN EVENTS ev2 ON ev2.objectid = ev1.objectid
AND ev2.eventid > ev1.eventid
AND ev2.value =1
INNER JOIN EVENTS ev3 ON ev3.objectid = ev1.objectid
AND ev3.eventid > ev2.eventid
AND ev3.value =0
WHERE ev1.objectid =13655
AND ev1.value =0
GROUP BY ev1.eventid

 * 
Query que sumariza todos os eventos, menos o ultimo conjunto
select MAX(subq3.id_ultimo), subq3.value_ultimo, subq3.clock_ultimo, subq3.total, subq3.tempo_ok, subq3.tempo_problem, subq3.total
  from (
select AVG(subq1.tempo_ok) as tempo_ok , AVG(subq1.tempo_problem) as tempo_problem, COUNT(subq1.id_inicio) as total 
     , max(subq2.eventid) as id_ultimo, subq2.value as value_ultimo, subq2.clock as clock_ultimo
  from (
SELECT ev1.eventid as id_inicio, ev1.clock as clock_inicio
     , min(ev2.eventid) id_final, ev2.clock as clock_final
     , ev2.clock - ev1.clock as tempo_ok
     , ev3.clock - ev2.clock as tempo_problem
  FROM `events` ev1
inner join events ev2
   on ev2.objectid = ev1.objectid
  and ev2.eventid > ev1.eventid
  and ev2.value = 1
inner join events ev3
   on ev3.objectid = ev1.objectid
  and ev3.eventid > ev2.eventid
  and ev3.value = 0
where ev1.objectid = 13655
and ev1.value = 0
group by ev1.eventid) subq1
inner join events subq2
   on subq2.objectid = 13655
group by subq2.value, subq2.clock
) subq3
group by total
...
<?php
	...
	$titulo 		= _zeT('ITIL Metrics');//'Zabbix-IS - Capacidade e Tendência';
	$page['title'] 		= $titulo;
	$page['file'] 		= 'zbxe-itil-mt.php';
